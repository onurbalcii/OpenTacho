...
[English](README.md) · [Türkçe](README.tr.md) · [Deutsch](README.de.md) · **Русский** · [Polski](README.pl.md) · [Português (Brasil)](README.pt-BR.md) · [Español](README.es.md) · [Français](README.fr.md) · [Italiano](README.it.md) · [Čeština](README.cs.md)
...
<p align="center">
  <img src="docs/screenshots/ru/mobile.png" width="250" alt="Второй экран на телефоне">
</p>
...
- **Планировщик рейса**: введите расстояние и срок доставки из предложения — приложение смоделирует перерывы и ежедневный/еженедельный
  ...
  и окно доставки берутся из игры, результат виден в строке заказа. Средняя скорость учится по одометру; **паром/поезд** на маршруте
  оценивается по времени маршрута (считается отдыхом), при ручном плане вводится отдельно.
...
- **Тяжесть нарушений**: каждое нарушение помечается по классам ЕС (незначительное / серьёзное / очень серьёзное); **виртуальные штрафы** (€, примерно) по желанию; кнопка **Оплатить** переносит их в игру — копия новейшего сохранения с вычтенным из банковского счёта штрафом записывается в новый слот «OpenTacho: штраф оплачен», который загружается из меню Загрузить.
- **Голосовые объявления**: короткие фразы офлайн-голосом Windows («до конца вождения 15 минут», «перерыв завершён» …), на любом языке приложения; отдельные регуляторы громкости для сигнала и объявлений.
...
- **Правила США (FMCSA HOS)** для American Truck Simulator, выбираются автоматически по игре (Настройки → Правила
  позволяют принудительно выбрать ЕС или США): **перерыв 30 мин после 8 ч** вождения, **11 ч** вождения в день,
  **10 ч** отдыха; необязательный слой США добавляет **14-часовое окно** (вождение заканчивается через 14 ч после
  первого выезда дня, перерывы его не продлевают), цикл **70 ч / 8 дней** и **34-часовой перезапуск** (пропускается одной
  кнопкой). Шкалы, планировщик, история, распечатки и голосовые сообщения следуют активному набору правил. Разделение
  отдыха в спальном отсеке (7/3) не моделируется.
...
- **Журнал заказов и экспорт**: каждый груз записывается по событиям телеметрии — маршрут, груз, км, счётчики
  вождения / перерывов / нарушений, доход и опоздание при доставке, штраф за отмену, штрафы игры. Список внизу панели
  истории; кнопки **Распечатка тахографа** (оформленный PDF + txt, как 24-часовая распечатка DTCO: шкала дня, отрезки, нарушения, заказы) и **CSV** (дни + заказы) записывают
  всё в папку `exports` рядом с exe.
...
- **Мини-панель**: глобальная горячая клавиша (по умолчанию `Ctrl + Num 0`, настраивается) меняет окно на
  небольшой индикатор поверх игры — значок статуса, остаток времени, следующий шаг, мини-полосы (блок · день ·
  отдых · неделя/цикл), игровые часы, флажки, срок доставки с вердиктом планировщика и кнопки Перерыв / Манёвры /
  Пропуск / Полный отдых / Еженедельный отдых — перетаскивайте куда угодно; прозрачность и размер (70–160 %) настраиваются.
- **Темы**: Ван Гог (по умолчанию, алгоритмически нарисованный фон в стиле *Звёздной ночи*
  ...
  Размещение изображения: заполнить, вписать, по центру, плитка или растянуть; своя тема использует плоский вид простой темы.
- **Языки**: русский, английский, турецкий, немецкий, польский, бразильский португальский, испанский, французский,
  итальянский, чешский — новый язык добавляется одним JSON-файлом.
- **Второй экран на телефоне или планшете** (Настройки → Приложение → *Использовать на телефоне или планшете*
  либо кнопка-телефон под шестерёнкой): включите доступ по локальной сети и отсканируйте QR-код — устройство в той же
  Wi-Fi показывает тахограф в браузере, без приложения: статус, остаток времени, шкалы, ход дня, заказ с ETA, флажки,
  события, а также вкладки истории и планировщика. Перерыв / Манёвры / Пропуск / Полный отдых работают с телефона
  (можно отключить); страница не даёт экрану погаснуть и подаёт сигнал/вибрацию при предупреждениях. Ссылка содержит
  случайный ключ, в интернет ничего не открывается, при выключенном доступе и после закрытия
  приложения ничего не прослушивается; после каждого запуска доступ выключен, каждое включение выдаёт новый код. Брандмауэр
  Windows спросит один раз — разрешите для частных сетей.
- **Проверка обновлений**: при запуске приложение запрашивает у GitHub последний выпуск (можно отключить) и при
  наличии новой версии показывает баннер с кнопкой загрузки; в Настройки → Приложение — номер версии, ручная проверка и ссылка.
...
Два набора правил, выбираются по игре или вручную (Настройки → Правила): правило ЕС (561/2006) — 4:30 вождения →
перерыв 45 минут → 4:30 вождения → 11 часов ежедневного отдыха, раздельный перерыв 15 + 30, раздельный ежедневный отдых
3 + 9 и необязательный недельный слой (56/90 ч, продление до 10 ч, сокращённый отдых 9 ч, рабочий день, еженедельный
отдых) — и правила FMCSA (США) для American Truck Simulator (8 ч / 30 мин / 11 ч / 10 ч, необязательное окно 14 ч,
70 ч / 8 дней, перезапуск 34 ч).

**Нужен ли интернет или аккаунт?**
Аккаунт не нужен; всё считается локально и офлайн, ничего не отправляется. Наружу уходят только необязательная
проверка обновлений (читает сведения о последнем выпуске с GitHub, можно отключить) и кнопка обратной связи,
открывающая веб-форму в браузере. Второй экран на телефоне/планшете остаётся внутри локальной сети.
...
имеют значения; American Truck Simulator использует тот же плагин — в ATS автоматически действуют правила США FMCSA
(8 ч / 30 мин / 11 ч / 10 ч, окно 14 ч, 70 ч / 8 дней); в Настройки → Правила можно выбрать любой набор.

**Телефон не открывает страницу / QR-код не работает.**
Оба устройства должны быть в одной локальной сети (тот же роутер или точка доступа телефона); гостевые сети с
изоляцией клиентов это блокируют. Разрешите OpenTacho в брандмауэре Windows для частных сетей (спросит при первом
включении); если адрес показывает не ту сеть, выберите другую в поле *Сетевой адрес*. *Новый код* делает старые
...
Это лёгкая альтернатива с открытым исходным кодом (MIT): наборы правил ЕС и США, без аккаунта,
...
app/                   содержимое окна: main.html, mini.html, mobile.html, assets/ (логотип, фон, звуки)
lang/                  en, tr, de, ru, pl, pt-BR, es, fr, it, cs — по одному JSON-файлу на язык
