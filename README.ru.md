...
Настройки → Приложение → **Отправить отзыв** открывает форму на языке приложения. Перед публикацией
своего форка замените ссылки в `FEEDBACK_URLS` в начале `OpenTacho.py` на свои.
